...
import {
  ...
  OMNIROUTE_API_KEY_ENV_KEY,
  OMNIROUTE_BASE_URL_ENV_KEY,
  OPENWIKI_OMNIROUTE_MODE_ENV_KEY,
  OPENWIKI_OMNIROUTE_BUDGET_ENV_KEY,
...
function isNonSecretDiagnosticKey(key: string): boolean {
  return (
    ...
    // OmniRoute base URL and routing knobs are operational config, not
    // credentials. OMNIROUTE_API_KEY is deliberately absent here so it stays
    // redacted.
    key === OMNIROUTE_BASE_URL_ENV_KEY ||
    key === OPENWIKI_OMNIROUTE_MODE_ENV_KEY ||
    key === OPENWIKI_OMNIROUTE_BUDGET_ENV_KEY ||
