...
- For Web Search, use direct API ingestion through openwiki_ingest_connector with connectorId "web-search". It uses the provider configured for the connector (Tavily by default, or Ollama), requires TAVILY_API_KEY or OLLAMA_API_KEY accordingly, reads configured queries (plus configured URLs when the Ollama provider is set), and writes web-search-results.json.
