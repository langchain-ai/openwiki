...
- For LangSmith, use direct API ingestion through openwiki_ingest_connector with connectorId "langsmith". It fetches recent root runs, error runs, and computed per-project stats for configured projects, truncates run payloads, and writes langsmith-results.json. It requires LANGSMITH_API_KEY.
