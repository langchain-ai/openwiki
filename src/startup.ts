...
export async function resolveStartupCommand(
  ...
  if (
    ...
    const hasProviderKey = hasProviderCredentials(provider);
    ...
    if (!hasProviderKey) {
      ...
      const credentialName =
        apiKeyEnvKey ?? `${getProviderLabel(provider)} credentials`;

      return {
        kind: "error",
        exitCode: 1,
        message: `${credentialName} is required for non-interactive runs. Run openwiki in an interactive terminal to save credentials.`,
      };
    }
  }
