...
- Purpose: these traces are how this application actually runs in production. Use them to help a future agent understand and work efficiently in THIS codebase (debugging, changes, prompt/tool tuning, latency/token work), not to produce a standalone report. Every runtime fact you record should make the code easier to reason about or change.
...
- Include a "Runtime findings & opportunities" section: a short, ranked list where each item pairs concrete trace evidence with the code that produces it and a specific, verifiable suggestion. Prioritize what only traces can reveal: latency hotspots, token sinks, redundant work (e.g. the same file read many times in a run), and tool/prompt friction (a tool retried after a bad first call points at an unclear tool description or schema). Include only findings the sample actually supports; when the sample shows none of something (e.g. no failing traces), say so plainly instead of inventing one.
- Also document the run shape (middleware/loop structure) and a cost/latency note, each labeled as observed over the sampled traces.
...
- Weave these facts into the existing architecture/component pages they concern (the tools page gets observed per-tool usage; the agent-loop page gets turn count and latency), with bidirectional links so an agent reading that code finds the runtime evidence and vice versa. Keep one consolidated Runtime behavior page; do not create a page per project.
