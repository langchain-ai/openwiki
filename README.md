...
openwiki integrations install pi
```

The supported targets are **IBM Bob**, **Codex**, **Claude Code**, **OpenCode**, **Cursor**, **Kiro**, and **Pi**. All install at user level by default, so one installation works from any Git repository. Project paths are resolved to their Git repository root. User-level OpenCode integrations live under `~/.config/opencode`, IBM Bob uses `~/.agents/skills` and `~/.bob/mcp.json`, Kiro uses `~/.kiro/skills` and `~/.kiro/settings/mcp.json`, and Pi uses `~/.pi/agent/skills` and `~/.pi/agent/extensions`. Restart the coding agent after installation, open the repository, and ask:
...
openwiki integrations install <bob|codex|claude|opencode|cursor|kiro|pi> [--project [path]]
openwiki integrations uninstall <bob|codex|claude|opencode|cursor|kiro|pi> [--project [path]]
