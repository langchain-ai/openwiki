...
### Living memory for your coding agent.
...
OpenWiki helps coding agents start informed in unfamiliar repositories and leave
useful discoveries for the next agent. It maintains a Markdown wiki grounded in
code, retrieves the explanations relevant to a task, and shows related source
changes and pending discoveries alongside them.

An agent can find how retries work, check what changed on main and its branch,
complete the task, and record a constraint it uncovered. Another agent can use
that discovery immediately. The next wiki update verifies and consolidates it
into maintained knowledge. The purpose is less repeated investigation and context
that survives a session; code and tests remain authoritative.

OpenWiki also supports personal knowledge wikis from connected sources and an
interactive visualizer for exploring either kind of wiki.

**OpenWiki gives you:**

- **Task-focused memory** through `orient`, `outline`, and `read`, with explanations, claims, source evidence, and relevant changes.
- **Shared discoveries** through `reflect`: pending findings travel with a PR and are consolidated during updates.
- **Maintained documentation**, generated and reconciled by a [Deep Agents](https://github.com/langchain-ai/deepagentsjs) documentation agent or your coding agent.
...
- **Living repository memory:** retrieve relevant pages and sections with main and branch context, leave discoveries for other agents, and consolidate them through ordinary wiki updates. Follow the [agent workflow](docs/agent-workflow.md) for a concrete example.
...
Already using a coding agent? [Install its integration](#coding-agent-integrations)
to consult an existing wiki or generate one using that agent's model session.
...
Update an existing wiki to reconcile source changes, stale claims, and pending reflections:
...
Install OpenWiki's MCP tools and skill in your coding agent to consult repository
memory during ordinary coding tasks and record useful discoveries. Reading and
recording need no generation run or OpenWiki model invocation. The same integration
can generate or update the wiki using the coding agent's model and repository tools.
...
The supported targets are **Codex**, **Claude Code**, **OpenCode**, and **Cursor**. All install at user level by default, so one installation works from any Git repository. Project paths are resolved to their Git repository root. User-level OpenCode integrations live under `~/.config/opencode`, OpenCode's global configuration directory on every supported platform. Restart the coding agent after installation and open the repository. For an existing wiki, try a task such as:

```text
Use OpenWiki to understand retry handling, check the current implementation,
and investigate why our retry budget behaves unexpectedly. Leave a reflection
if you discover useful knowledge missing from the wiki.
```

If the repository has no wiki yet, ask:

```text
Initialize this repository's OpenWiki from the current source and tests.
```

To reconcile existing memory, ask:

```text
Update this repository's OpenWiki for source changes and pending reflections.
```
...
After upgrading OpenWiki, rerun the install command to refresh the bundled skill,
then restart the coding agent. Repository setup maintains a small, self-contained
guidance block in both `AGENTS.md` and `CLAUDE.md`; detailed schemas stay in the
tools. If MCP is unavailable, `openwiki/quickstart.md` remains the file entry point.
...
The four tools support the loop **consult memory → verify and work → record
discoveries → consolidate**. Use whichever tool fits the context already known:

| Tool               | Input                         | Returns                                                                             |
| ------------------ | ----------------------------- | ----------------------------------------------------------------------------------- |
| `openwiki_orient`  | `{ root }`                    | Repository introduction and a complete page directory with titles and descriptions. |
| `openwiki_outline` | `{ root, page }`              | Page scope and section hierarchy with stable IDs and authored descriptions.         |
| `openwiki_read`    | `{ root, page, sections? }`   | Selected prose, its bindings, and supporting claims and code evidence.              |
| `openwiki_reflect` | `{ root, finding, evidence }` | ID and path of a new discovery with captured source evidence.                       |

For a retry task, the calls might look like this. `section_retries` represents
the actual ID returned by outline:
...
The read can show the wiki's three-attempt explanation, a main change raising the
limit to five, and a local edit raising it to seven. The agent follows the cited
source, verifies the current behavior, and completes its task. If it discovers
that the limit includes the initial call, it can leave:
...
The returned file travels with the task's PR. Another agent can retrieve the
pending finding before an update incorporates it into the wiki. See the
[complete walkthrough](docs/agent-workflow.md) for response excerpts and the
transition from branch work to shared knowledge.

Use the absolute Git repository root and wiki-relative page paths such as
`concepts/retries.md`. Pass section IDs from `outline` to `read`, or omit
`sections` to read the whole page. A parent includes its descendants, each
returned once. Call whichever tool fits what the agent already knows; reading
requires no generation run or model invocation.

Each response includes consolidated wiki knowledge in `longTerm`, changes on main
since the wiki's source checkpoint in `shortTerm`, and net branch/local changes
in `working`. Changes connect to affected pages in `orient`, sections in
`outline`, and claims in `read`. Inspect the returned resources or Git history to
verify affected evidence. `inCheckout` identifies main changes incorporated into
checkout history, even if local edits change them again.

Both `shortTerm` and `working` include pending `reflections` available in the
checkout. Findings shared on main are short-term; new branch or local findings
are working memory. They remain provisional and include progressively narrower
connections to pages, sections, and claims. Missing history leaves readable
findings available in an optional `unclassifiedReflections` fallback.

After a task produces useful new repository knowledge, call
`openwiki_reflect({ root, finding, evidence: [{ resource }] })`. OpenWiki captures
source versions and returns `{ id, path }` for a new JSON file under
`openwiki/.reflections/`. Include it with the task's changes to share the discovery
with other agents. See [reflections](docs/reflections.md) for the authoring standard,
storage contract, and consolidation workflow.

Each update evaluates every reflection present when it starts, including when
source code is unchanged. It incorporates useful findings through normal claim
and prose reconciliation, avoids duplicate knowledge, and discards unsupported
findings. Processed files disappear after successful persistence. Failed work
remains pending for retry; discoveries added during the run wait for the next
update. OpenWiki keeps no reflection outcome ledger or archive.

The overview is the opening prose from `quickstart.md`; page descriptions come
from OKF frontmatter, and section descriptions are maintained alongside claims. Retrieval
uses locally available Git history and honors `.openwikiignore`. An `unavailable`
layer explains missing comparison inputs; an empty `changes` list means the
comparison succeeded. Connections identify evidence to review, not claims proven
false. Verify relevant source before relying on the wiki for code changes.

Pages need complete section and binding metadata for `outline` and `read`.
Legacy pages gain that metadata when they participate in an OpenWiki update;
retrieval returns correction guidance until then. See [the retrieval contract](docs/retrieval.md)
for response examples and selection behavior.
...
Sections and prose bindings connect each claim to the passages that express it. Section IDs survive heading renames, and bindings preserve the exact text and supporting claim references without adding annotations to Markdown. Page authors submit only changed claims, sections, and bindings; invalid connections return feedback for correction while the page stays pending. Existing sidecars remain readable and gain these links when their pages are next submitted. See [page authoring](docs/page-authoring.md) for the native and MCP submission contract, location format, and retry behavior.
...
- **Agents know when to use it.** Code-mode setup maintains the same self-contained memory guidance in `AGENTS.md` and `CLAUDE.md`. Each names all four tools and a wiki fallback. OpenWiki replaces only its `<!-- OPENWIKI:START -->…<!-- OPENWIKI:END -->` block, preserving user instructions and links outside it.
...
- **No-op runs do not churn docs.** An update with no source, claim, or reflection work skips model work and leaves wiki content untouched while refreshing `.last-update.json` to record that the check ran.
