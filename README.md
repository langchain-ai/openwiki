...
Drop an `openwiki/SKILL.md` file in your repository to customize how OpenWiki behaves for that specific project — conventions, terminology, areas to focus on or avoid, anything you'd want a documentation writer joining the project to know upfront. OpenWiki reads it on every run (chat, `--init`, and `--update`, including the repository planner and page writers) and follows it in addition to its built-in instructions.
