...
- **Coding-agent integrations** for IBM Bob, Codex, Claude Code, OpenCode, Cursor, Kiro, Oh My Pi, and Antigravity CLI, plus a first-class Pi package, using the host's model and repository tools.
...
- **OpenWiki integrations:** run OpenWiki directly inside IBM Bob, Codex, Claude Code, OpenCode, Cursor, Kiro, Oh My Pi, Antigravity CLI, or Pi, using the coding agent's authenticated model and native repository tools while OpenWiki manages the documentation lifecycle.
...
OpenWiki is also a native [Pi package](https://github.com/earendil-works/pi) and
ships the canonical OpenWiki skill plus an extension that registers the same six
lifecycle tools. Install it into Pi's user package settings:

```sh
pi install npm:openwiki
```

The extension launches the package's own `openwiki` CLI through Node and keeps a
single stdio MCP server alive for the session; it does not depend on an
`openwiki` executable being on `PATH`. Project-local installs use
`pi install --local npm:openwiki`. Do not use `openwiki integrations install pi`.
