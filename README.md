...
- **Coding-agent integrations** for Codex, Claude Code, OpenCode, Cursor, and Kiro, using the host's model and repository tools.
...
- **OpenWiki integrations:** run OpenWiki directly inside Codex, Claude Code, OpenCode, Cursor, or Kiro, using the coding agent's authenticated model and native repository tools while OpenWiki manages the documentation lifecycle.
...
The supported targets are **Codex**, **Claude Code**, **OpenCode**, **Cursor**, and **Kiro**. All install at user level by default, so one installation works from any Git repository. Project paths are resolved to their Git repository root. User-level OpenCode integrations live under `~/.config/opencode`, while Kiro uses `~/.kiro/skills` and `~/.kiro/settings/mcp.json`. Restart the coding agent after installation, open the repository, and ask:
...
openwiki integrations install <codex|claude|opencode|cursor|kiro> [--project [path]]
openwiki integrations uninstall <codex|claude|opencode|cursor|kiro> [--project [path]]
