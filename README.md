...
`openwiki` creates initial documentation in `openwiki/` when no wiki exists. If `openwiki/` already exists, it refreshes that documentation from repository changes. To use a different repository-local documentation directory, set `OPENWIKI_DOCS_DIR` to a safe relative path such as `docs/openwiki`; absolute paths and paths that escape the repository are rejected. By default, the CLI stays open after each run so you can send follow-up messages. Use `-p` or `--print` for a one-shot non-interactive run that prints the final assistant output.
