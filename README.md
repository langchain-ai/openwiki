...
OpenWiki supports OpenRouter, z.ai, Fireworks, Baseten, OpenAI, an OpenAI-compatible provider, Anthropic, and local servers (Ollama, LM Studio, vLLM) out of the box. Local servers need no API key — set `OPENWIKI_PROVIDER=ollama` (or `lmstudio`/`vllm`) and `OPENWIKI_MODEL_ID` to a model your server has loaded, optionally overriding the endpoint with `OLLAMA_BASE_URL` / `LMSTUDIO_BASE_URL` / `VLLM_BASE_URL`. By default, there are a few models pre-defined (GLM 5.2, Kimi K2.6, Sonnet 5, etc) but for each inference provider, OpenWiki will allow you to specify your own custom model ID.
