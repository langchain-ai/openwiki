...
The Anthropic provider also accepts a Claude Code OAuth token instead of a raw
API key. Generate one with `claude setup-token` and set it as
`CLAUDE_CODE_OAUTH_TOKEN` (in the environment or `~/.openwiki/.env`):
