...
```sh
pnpm integrations:dev <bob|codex|claude|opencode|cursor|kiro|pi>
```
...
and Pi install at user scope. Later source changes only require `pnpm build`
...
Kiro under `~/.kiro`, and Pi under `~/.pi/agent`.
