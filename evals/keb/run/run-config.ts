...
import type { KebRunConfig } from "../core/types.js";
 ...
 * `KEB_EVALUATOR_MODEL_ID`, falling back to the system model. The evaluator
 ...
 * @param evalDir - Absolute path to the `evals/keb` directory, used to resolve
 *   the default results directory.
 *
 * @returns The resolved run config.
 *
 * @throws KebError when a required value is missing.
 */
export function resolveRunConfig(
  ...
  const evaluatorModelId =
    args.evaluatorModel ?? env.KEB_EVALUATOR_MODEL_ID ?? systemModelId;

  if (evaluatorModelId === undefined) {
    throw new KebError(
      "An evaluator model id is required. Set --evaluator-model, KEB_EVALUATOR_MODEL_ID, or a system model id it can fall back to.",
    );
  }
