 ...
 *
 * @throws KebError when a flag is unknown or lacks a value.
 */
 ...
 * @param evalDir - Absolute `evals/keb` directory used for default results.
 *
 * @returns Complete evaluator-only configuration.
 *
 * @throws KebError when a required benchmark, run, provider, or model is absent.
 */
export function resolveReevaluationConfig(
  ...
  if (args.benchmark === undefined) {
    throw new KebError("A benchmark directory is required: --benchmark <dir>.");
  }

  if (args.run === undefined) {
    throw new KebError("A completed run directory is required: --run <dir>.");
  }

  const provider = env.OPENWIKI_PROVIDER;

  if (provider === undefined || provider.length === 0) {
    throw new KebError("OPENWIKI_PROVIDER must be set in the environment.");
  }

  const evaluatorModelId =
    args.evaluatorModel ?? env.KEB_EVALUATOR_MODEL_ID ?? env.OPENWIKI_MODEL_ID;

  if (evaluatorModelId === undefined || evaluatorModelId.length === 0) {
    throw new KebError(
      "An evaluator model id is required. Set --evaluator-model, KEB_EVALUATOR_MODEL_ID, or OPENWIKI_MODEL_ID.",
    );
  }
