...
import { KebError, WorktreeSafetyError } from "../core/errors.js";
...
/**
 * Parse one saved JSON file and wrap malformed input in an actionable KEB error.
 *
 ...
 *
 * @throws KebError when the file cannot be read or parsed.
 */
 ...
 *
 * @throws KebError when a required field is absent or inconsistent.
 */
...
/**
 * Load one immutable generated knowledge artifact from a completed KEB run.
 * Every manifest path is resolved beneath the checkpoint snapshot directory
 * before it is read.
 *
 * @param runDir - Directory containing saved KEB run artifacts.
 * @param checkpointId - Checkpoint artifact to load.
 *
 * @returns The reconstructed immutable artifact.
 *
 * @throws KebError when the manifest or a document cannot be read.
 ...
 *
 * @throws KebError when the corpus is malformed or belongs to another checkpoint.
 */
 ...
 *
 * @throws KebError when the result does not have the minimum required shape.
 */
export async function loadSavedRunResult(
  ...
  if (
    ...
    throw new KebError(`Saved run result "${file}" is malformed.`);
  }

  return value as unknown as KebRunResult;
}
