 ...
 *
 * @returns A serializable empty KEB result.
 */
function result(startedAt: string): KebRunResult {
  return {
    metadata: {
      benchmarkName: "saved",
      startedAt,
...
describe("saved run inputs", () => {
  ...
  test("round-trips immutable artifacts, evidence, and run metadata", async () => {
    const resultsDir = await mkdtemp(path.join(os.tmpdir(), "keb-saved-"));
  ...
  test("rejects a document path that escapes its saved snapshot", async () => {
    const runDir = await mkdtemp(path.join(os.tmpdir(), "keb-saved-"));
