...
describe("runBenchmark", () => {
  ...
  function benchmark(): KebBenchmark {
    return {
      name: "fake",
  ...
  test("produces the exact hand-computed KEB score", async () => {
    ...
    expect(result.score.kebScore).toBeCloseTo(13 / 14, 10);
...
describe("runBenchmark forgetting watch set", () => {
  ...
  function benchmark(): KebBenchmark {
    return {
      name: "watch-set",
      description: "three checkpoints exercising carry-forward and revival",
      sourceRepoPath: repo.repoPath,
      trace: {
        checkpoints: [
          { id: "T0", commit: repo.shas[0] },
  ...
  test("keeps a forgotten obsolete version under watch and drops a revived one", async () => {
    ...
    // At T2: f2@T0 was judged forgotten at T1 but is still watched, because KEB
