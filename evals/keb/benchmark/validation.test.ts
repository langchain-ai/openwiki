...
import type { KebBenchmark, KebCheckpoint, TruthFact } from "../core/types.js";
 ...
 */
function expectRejected(benchmark: KebBenchmark, messagePattern: RegExp): void {
...
describe("validateBenchmark trace rules", () => {
  ...
  test("rejects a null checkpoint entry as a validation error, not a crash", () => {
    const benchmark = valid();
    benchmark.trace.checkpoints[0] = null as unknown as KebCheckpoint;
