...
describe("isContainedBy", () => {
  ...
  test("accepts a directly nested child", () => {
    expect(isContainedBy(root, "/tmp/keb-workspace/wt")).toBe(true);
  });

  test("accepts a deeply nested descendant", () => {
    expect(isContainedBy(root, "/tmp/keb-workspace/wt/openwiki/a/b")).toBe(
      true,
    );
  });

  test("accepts a child reached via .. that resolves back inside the root", () => {
    expect(isContainedBy(root, "/tmp/keb-workspace/wt/../openwiki")).toBe(true);
  });
  ...
  test("rejects a sibling that merely shares a string prefix", () => {
    ...
    expect(isContainedBy(root, "/tmp/keb-workspace-evil")).toBe(false);
    expect(isContainedBy("/tmp/keb", "/tmp/keb-other")).toBe(false);
  });

  test("rejects a child that climbs out of the root via ..", () => {
    expect(isContainedBy(root, "/tmp/keb-workspace/../keb-other")).toBe(false);
  });

  test("rejects .. resolving to the parent even when spelled as a child", () => {
    // "/tmp/keb-workspace/.." normalizes to "/tmp", the parent, so it must be
    // rejected despite being written as a path underneath the root.
    expect(isContainedBy(root, "/tmp/keb-workspace/..")).toBe(false);
  });

  test("accepts a real child whose name merely begins with .. (regression)", () => {
    // path.relative(root, "/tmp/keb-workspace/..cache") === "..cache". A naive
    ...
    expect(isContainedBy(root, "/tmp/keb-workspace/..cache")).toBe(true);
    expect(isContainedBy(root, "/tmp/keb-workspace/wt/..config/x")).toBe(true);
  });
});
