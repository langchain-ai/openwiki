 ...
 * behaviourless `extends KebError {}`, so one table drives the name, message, and
 * `instanceof` assertions for all of them.
 */
const SUBCLASSES: ReadonlyArray<{
  readonly name: string;
  readonly construct: (message: string) => KebError;
}> = [
  {
...
describe("KebError", () => {
  test("is an Error whose name is its own class name", () => {
    const error = new KebError("base failure");

    expect(error).toBeInstanceOf(Error);
    expect(error.name).toBe("KebError");
...
describe.each(SUBCLASSES)("$name", ({ name, construct }) => {
  ...
  test("is catchable as both KebError and Error", () => {
    const error = construct("boom");

    expect(error).toBeInstanceOf(KebError);
    expect(error).toBeInstanceOf(Error);
  });
});
