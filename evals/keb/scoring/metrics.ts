...
/**
 * Aggregate per-checkpoint scores into the final KEB Score. Trace Coverage and
 ...
 * defined rates are averaged for the Maintenance Score. The KEB Score is the mean
 ...
 */
export function aggregateScore(checkpoints: CheckpointScore[]): KebScore {
  ...
  const kebScore =
    maintenance === undefined ? quality : (quality + maintenance) / 2;
...
export function computeRecoveryRate(
  ...
  // rather than accumulated. KEB does not treat forgetting as permanent: an
