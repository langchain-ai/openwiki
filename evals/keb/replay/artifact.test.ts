...
describe("captureArtifact", () => {
  ...
  async function scratch(
    ...
    const worktreeDir = await mkdtemp(path.join(os.tmpdir(), "keb-wt-"));
    const artifactsRoot = await mkdtemp(path.join(os.tmpdir(), "keb-art-"));
  ...
  test("does not capture a symlink inside the wiki that points outside it", async () => {
    ...
    const outside = await mkdtemp(path.join(os.tmpdir(), "keb-out-"));
