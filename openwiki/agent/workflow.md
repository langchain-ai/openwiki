...
OpenWiki owns the OKF v0.2 `generated` provenance field end to end: the model never authors it (the prompts instruct the agent not to write, edit, or remove `generated` or the legacy `timestamp`), so freshness never rests on a hallucinated date. The OKF middleware's `wrapToolCall` stamps it on every concept write whose body meaningfully changes, via `stampGeneratedProvenance()` in `src/agent/okf-middleware.ts`:

- It reads the target's body just before the write (`readConceptContent()`), then re-reads after the write.
- A newly created concept (no prior body) is always stamped. A write whose body is unchanged after `conceptBodiesEqual()` (front matter stripped, whitespace collapsed) leaves `generated` alone — a metadata-only or whitespace reflow must not bump the recorded change time.
- On a meaningful change it writes `generated: {by: openwiki/<version>, at: <run time>}` (via `setGeneratedEvent()`) and drops any superseded legacy `timestamp` (`removeFrontmatterField()`). The `by` actor comes from `OPENWIKI_PRODUCER_ACTOR` in `src/version.ts` (`openwiki/${OPENWIKI_VERSION}`); the `at` time is the run's single `runTimestamp` threaded from `createOpenWikiAgentGraph()`.
- Re-stamping within one run (same `now`, no `timestamp`) is a no-op and the redundant write is skipped so idempotent rewrites do not churn the file.
- Reserved `index.md`/`log.md` and non-`.md` paths are skipped by `isWikiMarkdownPath()`.
- Stamping is best-effort: any error is logged and swallowed, leaving the page validly unstamped (`generated` is optional in OKF v0.2) for a later body-changing update to stamp. This guard is load-bearing — the tool node re-raises a `wrapToolCall` throw as a fatal `MiddlewareError`, so without it a failed provenance write would fail an otherwise-successful run.

The stamping behavior matrix is covered by `test/agent/index-middleware.test.ts` ("createOpenWikiIndexMiddleware wrapToolCall generated stamping": "stamps a run-timed generated event on a newly created concept", "bumps at and drops legacy timestamp when the body changes", "does not bump generated when only the body whitespace changes", "stamps a nested concept but never a reserved document", "does not stamp a non-Markdown file written under the wiki", "keeps an otherwise-successful write when the stamp write fails"). The front-matter helpers are covered by `test/okf/frontmatter.test.ts` (`setGeneratedEvent`, `conceptBodiesEqual`, `normalizeConceptContent` "carries a code-owned generated event across a regeneration").
...
- The OKF v0.2 `generated` provenance field is code-owned, not model-authored. Do not add prompt text that tells the model to write `generated: {by, ...}` or `timestamp`; both are owned by `stampGeneratedProvenance()` in `src/agent/okf-middleware.ts`. The producer actor must derive from `OPENWIKI_PRODUCER_ACTOR` in `src/version.ts` (single source) so the prompt and the stamp can never drift — `test/agent/prompt-okf.test.ts` asserts the prompt never contains the resolved actor or a `generated: {by:` template. The `wrapToolCall` stamp path must stay best-effort (errors swallowed, never rethrown): the tool node re-raises a `wrapToolCall` throw as a fatal `MiddlewareError`, so turning the catch into a rethrow would make every recoverable stamp failure fail the run — asserted by "keeps an otherwise-successful write when the stamp write fails".
