...
9. Create a DeepAgents `LocalShellBackend` rooted at the repository with a SQLite checkpointer, wrap it in a `CompositeBackend` via `createAgentBackend()` (an `OpenWikiCompositeBackend` subclass that turns a `RangeError` from an over-broad `glob` into a tool error rather than crashing the run), then attach OKF index middleware (`src/agent/okf-middleware.ts`) and translation middleware (`src/agent/translation-middleware.ts`). The `/conversation_history/` mount routes the DeepAgents summarization middleware's history offload to `~/.openwiki/conversation_history` so it succeeds even on docs-only runs (without it, the docs-only guard refuses the offload and summarization silently degrades — #496). `AGENT_FILESYSTEM_PERMISSIONS` denies the model's own writes to both `/skills/**` and `/conversation_history/**`. For repository init/update runs, `createClaimsIntegration()` adds the `resolve_claims`, `inspect_claims`, and Claims-aware `delete_file` tools plus the Claims read-note middleware to the agent graph (see [Grounded Claims](../claims/grounded-claims.md)). For `init` repository runs, the agent graph also registers init-only review subagents (see [Init subagents](#init-subagents)). The OKF middleware migrates front matter before the agent runs (via the shared `prepareWikiForAuthoring()` in `src/agent/wiki-finalizer.ts`), validates writes, and synchronizes `index.md` files after; its finalize stage (via `finalizeWikiArtifacts()`) also validates Mermaid fences, validates internal wiki links, projects Claims evidence into OKF `sources`, and finalizes `generated` provenance. The translation middleware translates eligible pages when the output language has changed. One `runTimestamp` (`new Date().toISOString()`) is computed per run and threaded into both the middleware factory and the Claims runtime as the shared provenance/verification time, so every page whose body changes and every verified Claims page shares a single deterministic timestamp.
10. Stream messages and tool events back to the CLI. The run consumes the graph with `agent.stream(input, { streamMode, subgraphs: true })` rather than the older `streamEvents` v3 protocol. `streamMode` is conditional: most providers use `["messages", "tools"]`, but the `openai-compatible` provider defaults to `["updates", "tools"]` (resolved in `src/agent/index.ts` via `streamMessagesEnabled = provider !== "openai-compatible" || resolveOpenAiCompatibleStreamMessages()`). The `messages` stream mode forces `@langchain/core` to route `.invoke()` through `_streamResponseChunks` chunk aggregation; endpoints that stream reasoning deltas before the first `role:"assistant"` delta (z.ai GLM) aggregate to a `ChatMessageChunk`, which the agent loop's `wrapModelCall` validator rejects (`expected AIMessage or Command, got object`, issue #659). Dropping `messages` keeps the model on the non-streaming `_generate` path, which returns a proper `AIMessage` — at the cost of no live token streaming for openai-compatible runs. Endpoints known to emit a `role:"assistant"` first delta can opt back into live streaming with `OPENWIKI_OPENAI_COMPATIBLE_STREAM_MESSAGES=true` (resolved by `resolveOpenAiCompatibleStreamMessages()` in `src/config/constants.ts`, default false). `parseAgentStreamChunk()` in `src/agent/index.ts` normalizes each `[namespace, mode, payload]` chunk into an `OpenWikiRunEvent`: `tools`-mode chunks become tool start/end events, `messages`-mode chunks become text events with `source: "main"` when the namespace is a single segment or `source: "subgraph"` when deeper (task output). `extractMessageText()` filters out non-text content blocks — `tool`, `reasoning`, `file`, and `image` types — so raw base64 payloads from file/image blocks never leak into the terminal output. A `scheduler.yield()` between chunks lets Ink paint streamed text before the async iterator advances. (`parseStreamEvent()` is retained for the public agent factory's Agent Protocol event shape, but the live run uses `parseAgentStreamChunk()`.) With `updates` mode the model no longer streams live token deltas to the TUI (the documented cost of the safe path), so streamed text for openai-compatible default runs arrives with the update aggregation rather than token-by-token. Before iterating, the runtime calls `registerActiveRun()` from `src/agent/crash-guard.ts` with the command, cwd, model id, output mode, pre-run snapshot, and language, so a rejection that escapes the for-await catch (e.g. a subagent error surfacing on the microtask queue) is still attributed; `clearActiveRun()` runs in the `finally`.
11. For `init` and `update`, compare the post-run content snapshot to the pre-run snapshot and persist run metadata via `persistRunMetadataIfChanged()` in `src/agent/utils.ts`. The finalize stage now runs in a try/catch: it cleans up temporary working files (`removeTemporaryWorkingFiles`), finalizes the Claims runtime (`claimsRuntime.finalize(runTimestamp)`, which persists dirty sidecars, projects OKF `sources`/`verified`, and cleans orphans), then persists run metadata. If the finalize stage throws, metadata is persisted with `status: "interrupted"` so the next update retries. Metadata is now **always refreshed for non-chat runs** (not only when content changed) so freshness checks reflect the actual last run; a completed retry that changed no content still clears a previous `interrupted` status. If the run fails mid-stream, the catch block writes metadata with `status: "interrupted"` so the next update retries instead of skipping as a no-op. After the run (success or failure), `recordRunSafe()` in `src/telemetry/` emits a single `openwiki_run` PostHog event with mode, provider, outcome, and latency. A rejection that escapes every catch is caught by the process-wide crash guard (see [Crash guard](#crash-guard)).

Chat runs skip metadata writes entirely.

## Provider-specific model creation

`createModel()` in `src/agent/index.ts` branches by provider. It now takes `maxOutputTokens?` and `streamIdleTimeout?` (threaded from `resolveRunConfig()`, which resolves `OPENWIKI_MAX_OUTPUT_TOKENS` via `resolveMaxOutputTokens()` and the Bedrock-only `OPENWIKI_STREAM_IDLE_TIMEOUT` via `resolveStreamIdleTimeoutForProvider()`), and it consults `resolveReasoningConfig()` from `src/config/reasoning.ts` for the `OPENWIKI_REASONING_EFFORT` opt-in. Each branch spreads only the relevant options:
...
- **baseten / fireworks / nebius / nvidia / openai-compatible**: `new ChatOpenAI({ apiKey, configuration: { baseURL? }, model, useResponsesApi: providerUsesResponsesApi(provider, modelId) })` — OpenAI-compatible clients using the provider's base URL when configured. The `openai-compatible` provider has no default endpoint; its base URL is user-supplied via `OPENAI_COMPATIBLE_BASE_URL` and required (`requiresBaseUrl: true`), which lets OpenWiki target any OpenAI-compatible gateway (for example a LiteLLM gateway fronting upstream providers). It defaults to standard chat completions (`useResponsesApi: false`) so it works against gateways that only implement `/chat/completions`; setting `OPENWIKI_OPENAI_COMPATIBLE_USE_RESPONSES_API=true` opts it into OpenAI's Responses API via `resolveOpenAiCompatibleUseResponsesApi()` in `src/config/constants.ts` (the other providers in this group have no `responsesApi` config, so `providerUsesResponsesApi()` resolves to `false` for them). When `OPENWIKI_OPENAI_COMPATIBLE_STREAMING=true`, `providerUsesStreaming()` resolves the HTTP-streaming opt-in and `createModel()` spreads `streaming: true` so every generation (including DeepAgents' non-streaming `.invoke()` calls) uses SSE — a separate axis from the stream mode / stream-messages behavior, needed because some gateways serve only the streaming transport and return HTTP 200 with empty content otherwise (#655).
...
`OPENWIKI_REASONING_EFFORT` configures reasoning for a supported provider and model. The capability table lives in `src/config/reasoning.ts`:

- OpenAI GPT-5.6 models (`gpt-5.6-terra`/`luna`/`sol`) on the `openai` and `openai-chatgpt` providers use the Responses-API reasoning payload (`reasoning: { effort }`) with values `none`, `low`, `medium`, `high`, `xhigh`, `max`.
- NVIDIA NIM's `nvidia/nemotron-3-super-120b-a12b` uses chat-completions `reasoning_effort` (`modelKwargs.reasoning_effort`) with values `none`, `low`, `high`.

`resolveReasoningConfig(provider, modelId)` returns `undefined` when the variable is unset (provider default preserved) and otherwise validates the value against the capability table, throwing before a request is sent for an invalid effort, an unsupported provider/model, or a value the model does not expose. In `createModel()`, the resolved effort is spread as `responsesReasoningOptions` (`reasoning: { effort }`, cast narrowly at the `ChatOpenAI` constructor boundary because `max` is not yet in every OpenAI SDK type union) for the Responses transport, or as `chatCompletionsReasoningOptions` (`modelKwargs: { reasoning_effort }`) for the chat-completions transport — touching the `openai`, `openai-chatgpt`, `openai-compatible`, and `nvidia` branches. Behavior is covered by `test/agent/create-model.test.ts` ("createModel reasoning configuration": "maps OpenAI GPT-5.6 effort to the Responses reasoning payload", "serializes OpenAI GPT-5.6 effort in the Responses request", "maps NVIDIA NIM effort to the Chat Completions request field") and `test/config/constants.test.ts` ("reasoning capabilities"). Interactive selection is described in [CLI usage § Reasoning effort](../cli/usage.md#reasoning-effort).
...
That metadata is later used to scope update runs. When a run fails mid-stream, the catch block in `src/agent/index.ts` calls `persistRunMetadataIfChanged()` with `status: "interrupted"`, so already-generated content stays diffable. A rejection that escapes every catch is caught by the process-wide [crash guard](#crash-guard), which records the failure and stamps the same interrupted status post-mortem. `getUpdateNoopStatus()` then sees the interrupted status and does not skip the next update — preventing a possibly partial wiki from being treated as current. Metadata without a `status` field (from older versions) is treated as `"complete"`. A completed retry that changes no content still rewrites metadata to clear the interrupted status. `getUpdateNoopStatus()` also does not skip when the requested output language's primary subtag differs from the persisted wiki language (reason `"output language changed"`), so a `--language` switch on a clean tree still runs the translation pass before the update agent — see [Credentials and updates § Interrupted runs](../operations/credentials-and-updates.md#interrupted-runs). The subtag comparison uses the shared `getPrimaryLanguageSubtag()` in `src/platform/language.ts`.
...
### Generated provenance stamping

OpenWiki owns the OKF v0.2 `generated` provenance field end to end: the model never authors it (the prompts instruct the agent not to write, edit, or remove `generated` or the legacy `timestamp`), so freshness never rests on a hallucinated date. Provenance is finalized in a snapshot-and-reconcile pass owned by `src/okf/generated-provenance.ts`, run from the shared wiki finalizer (`src/agent/wiki-finalizer.ts`) invoked by the OKF middleware's lifecycle hooks:

- In `beforeAgent`, `snapshotGeneratedProvenance()` walks every current concept (via `listWikiConceptPaths()` in `src/okf/index-sync.ts`) and records a `ConceptSnapshot` keyed by virtual path: a SHA-256 hash of the exact Markdown body (front matter stripped, **whitespace retained**) plus the parsed prior `generated` event when one is valid. Only hashes and the prior event are retained, so the snapshot is bounded without writing temp files beside the wiki.
- The agent then writes freely; the `wrapToolCall` hook only decorates successful tool results with front-matter warnings and no longer touches `generated`.
- In `afterAgent`, after Mermaid/link/index validation has post-processed the wiki (run through `finalizeWikiArtifacts()` in `src/agent/wiki-finalizer.ts`), Claims evidence is projected into OKF `sources` front matter (`synchronizeClaimSources()` in `src/okf/claim-sources.ts`), and then `finalizeGeneratedProvenance()` re-walks every concept and reconciles each against its pre-run snapshot:
  - A **new** page (no snapshot) or a page whose body hash differs in **any way** — including a whitespace-only reflow — receives the run stamp: `generated: {by: openwiki/<version>, at: <run time>}` (via `setGeneratedEvent()`) with the superseded legacy `timestamp` dropped (`removeFrontmatterField()`). The `by` actor comes from `OPENWIKI_PRODUCER_ACTOR` in `src/version.ts` (`openwiki/${OPENWIKI_VERSION}`); the `at` time is the run's single `runTimestamp` threaded from `createOpenWikiAgentGraph()`.
  - An **unchanged** body gets its prior `generated` event restored if an agent rewrite removed or altered it (`restoreGeneratedEvent()`); a page that was previously unstamped stays unstamped. This is why finalization runs _after_ post-processing: an agent's full-file rewrite commonly drops the code-owned field, and the snapshot lets the finalize pass put it back exactly.
  - A write whose body is byte-identical but whose front matter changed (e.g. a `type`/`title` edit) does **not** bump `generated` — only the body hash gates the stamp.
- Reserved `index.md`/`log.md` and non-`.md` paths are excluded by `listWikiConceptPaths()`, so they are never stamped.
- Unlike the prior per-write path, finalization **throws** when a stamp write cannot be persisted (`Unable to finalize generated provenance for <path>: <error>`): the model-authored content survives on disk, but the run is marked failed because its code-owned provenance is absent. The `afterAgent` stage is wrapped in `inStage("finalize", …)` so the failure is classified as `okf_error`/`generated_provenance` rather than a generic tool error.

The finalization behavior matrix is covered by `test/agent/index-middleware.test.ts` ("createOpenWikiIndexMiddleware generated finalization": "stamps a newly created concept after its final full-file rewrite", "bumps at and drops legacy timestamp when the body changes", "bumps generated when only the body whitespace changes", "does not bump generated when only front matter changes", "stamps a nested concept but never a reserved document", "does not stamp a non-Markdown file written under the wiki", "leaves a preserved unstamped page unstamped", "fails finalization when the generated event cannot be persisted"). The front-matter helpers are covered by `test/okf/frontmatter.test.ts` (`setGeneratedEvent`, `removeFrontmatterField`, `normalizeConceptContent` "carries a code-owned generated event across a regeneration").

## Claims finalization

After wiki artifact finalization (Mermaid, index sync, links, sources, generated provenance), `ClaimsRuntime.finalize()` runs in the same `finalize` stage. It persists only dirty sidecars, rechecking each against current evidence and hashing its Markdown before writing; orphaned sidecars (whose pages were deleted) are removed. Page-local persistence failures are isolated as warnings so one page's problem does not abort the run, but unresolved evidence debt or a non-recoverable error does throw, marking the run interrupted. A successful finalization stamps a `verification: {by, at}` event into each page's sidecar, projected into OKF `verified` front matter by `synchronizeClaimsVerification()` in `src/okf/claims-verification.ts`. See [Grounded Claims § Finalization behavior](../claims/grounded-claims.md#finalization-behavior) for the full behavior matrix.
...
- avoid metadata drift via the content-snapshot check,
- support both interactive and scheduled maintenance use cases.

The same agent runtime is the wiki-generation backend invoked by two sibling evaluation harnesses: the [DeepSWE evaluation harness](../evals/deepswe-harness.md), which runs it in an isolated clone to produce treatment wikis for paired benchmark trials, and the [LEDGER longitudinal benchmark](../evals/ledger-harness.md), which replays a source repository's Git checkpoints and drives `init` then `update` at each through its `OpenWikiSystem` adapter (`evals/ledger/system/openwiki-system.ts`, calling `runOpenWikiAgent` with `outputMode: "repository"`) to measure per-claim grounding and forgetting. Both harnesses are downstream consumers of this runtime, so a behavior change here affects both benchmark surfaces.
...
- Be careful with `.last-update.json` semantics, because update runs use it to decide what changed since the previous successful run. The `status` field (`"complete"` / `"interrupted"`) gates the no-op skip: `getUpdateNoopStatus()` does not skip when the previous run was interrupted, and a completed retry clears the status even without content changes. `persistRunMetadataIfChanged()` now always refreshes metadata for non-chat runs (not only when content changed) so freshness checks reflect the actual last run — the fast-skip path in `runOpenWikiAgent` also calls `writeLastUpdateMetadata()`, carrying the persisted `language` surfaced by `getUpdateNoopStatus()` so a non-English wiki keeps its marker. A requested `--language` whose primary subtag differs from the persisted wiki language also defeats the skip (reason `"output language changed"`), so the translation pass runs before the update agent; `getUpdateNoopStatus()` now takes a `requestedLanguage` argument threaded from both the agent's update gate and the CLI's pre-credential `canSkipCleanUpdateBeforeCredentials()` in `src/cli/startup.ts`. Both the catch block and the [crash guard](#crash-guard) stamp interrupted; keep both in sync if metadata semantics change.
...
- The DeepAgents backend is configured with `virtualMode: true`, which is important for documentation-only behavior. The custom `OpenWikiLocalShellBackend` in `src/agent/docs-only-backend.ts` adds docs-only write guards that restrict writes to the `openwiki/` directory in docs-only mode. Its `glob` override also rejects unbounded root globs (`**`, `**/*`, `**/**` at the repository root) and globs targeting `.git` metadata, and recovers the worktree `ENOTDIR` scandir error raised when a glob scans a file-backed `.git` pointer as a directory — covered by `test/agent/docs-only-backend.test.ts` ("handles file-backed Git worktree metadata without crashing glob").
...
- `src/version.ts` (exports `OPENWIKI_PRODUCER_ACTOR`, the `by` actor stamped on `generated` events)
