...
10. For `init` and `update`, compare the post-run content snapshot to the pre-run snapshot and persist run metadata via `persistRunMetadataIfChanged()` in `src/agent/utils.ts`. Metadata is now **always refreshed for non-chat runs** (not only when content changed) so freshness checks reflect the actual last run; a completed retry that changed no content still clears a previous `interrupted` status. If the run fails mid-stream, the catch block writes metadata with `status: "interrupted"` so the next update retries instead of skipping as a no-op. After the run (success or failure), `recordRunSafe()` in `src/telemetry/` emits a single `openwiki_run` PostHog event with mode, provider, outcome, and latency. A rejection that escapes every catch is caught by the process-wide crash guard (see [Crash guard](#crash-guard)).
...
- In `afterAgent`, after Mermaid/link/index validation has post-processed the wiki, `finalizeGeneratedProvenance()` re-walks every concept and reconciles each against its pre-run snapshot:
  ...
  - An **unchanged** body gets its prior `generated` event restored if an agent rewrite removed or altered it (`restoreGeneratedEvent()`); a page that was previously unstamped stays unstamped. This is why finalization runs *after* post-processing: an agent's full-file rewrite commonly drops the code-owned field, and the snapshot lets the finalize pass put it back exactly.
