...
1. `src/cli/cli.tsx` is the CLI entrypoint: it loads env, parses the command, installs the crash guard, and dispatches to the interactive Ink `App` (`src/cli/app/app.tsx`) or the non-interactive runners in `src/cli/runners.ts`.
...
3. `src/setup/credentials.tsx` is a thin composition root for the interactive onboarding flow (provider selection, API keys, model selection, optional LangSmith tracing); its state machine lives in `src/setup/credentials/use-init-setup.ts`, rendering in `view.tsx`, and pure helpers in `steps.ts`, `format.ts`, `persistence.ts`, and `constants.ts`.
...
10. `src/auth/` contains the connector OAuth system: `oauth.ts` (generic runner), `oauth-discovery.ts` (OAuth metadata discovery and endpoint validation), `providers.ts` (provider configs), `configure.ts` (`openwiki auth configure`), `ngrok.ts` (Slack HTTPS tunnel), `tokens.ts` (refresh/validation), `external-cli-auth.ts` (copilot GitHub-CLI auth), and `types.ts`.
11. `src/connectors/` contains the connector registry, MCP client/runtime, a shared resilient HTTP helper (`http.ts`), connector config parsing (`config.ts`), source-specific ingestion modules (git-repo, gmail, hackernews, slack, web-search, x), and tool definitions exposed to the agent.
...
`shouldAutoExitStartupRun()` in `src/cli/run-mode.ts` determines whether a startup run should exit automatically on success. The Ink `App` in `src/cli/app/app.tsx` checks it when launching a run; this applies to `--init` and `--update` commands (without `--print`) when run in a TTY: the CLI launches the run, renders streaming output, and exits with code 0 on success. Chat runs and `--print` runs are unaffected.
...
- Add or refine CLI commands in `src/cli/commands.ts` and the corresponding UI behavior in `src/cli/app/app.tsx` and `src/cli/runners.ts`.
- Change onboarding or local credential storage in `src/setup/credentials.tsx` (and its `credentials/` submodules) and `src/config/env.ts`.
...
- **Diagnostics and redaction** (`src/platform/diagnostics.ts`): redacts secrets from error messages, headers, and provider responses before they are shown to the user or written to logs. It matches exact secret values from the environment and known token shapes (`sk-…`, `Bearer …`, `ls…`). CLI-facing diagnostic shaping (auth-fix hints, error-diagnostic formatting, header/control-character sanitization) lives under `src/cli/diagnostics/`.

## Things to watch when editing

- `src/cli/cli.tsx`, `src/cli/commands.ts`, `src/cli/app/app.tsx`, and `src/cli/runners.ts` must stay aligned; help text, parser behavior, and command dispatch are intentionally coupled.
...
- When adding a provider, update `MANAGED_ENV_KEYS` in `src/config/env.ts` so diagnostics and env formatting cover the new key.
...
- `src/cli/cli.tsx`, `src/cli/commands.ts`, `src/cli/app/app.tsx`, `src/cli/runners.ts`, `src/cli/run-mode.ts`, `src/cli/startup.ts`
- `src/cli/components/` (chat, header, markdown, panels, primitives, run-view, first-run-notice, types)
...
- `src/setup/credentials.tsx` and `src/setup/credentials/` (components, constants, format, persistence, steps, types, use-init-setup, view)
- `src/setup/onboarding.ts`
- `src/config/env.ts`, `src/config/constants.ts`, `src/config/openwiki-home.ts`
- `src/agent/index.ts`
- `src/agent/prompt.ts`
...
- `src/telemetry/` (including `errors.ts`, `gates.ts`, `record-run-safe.ts`, `senders.ts`, `client.ts`, `taxonomy.ts`)
- `src/auth/oauth.ts`
